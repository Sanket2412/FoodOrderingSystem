...
<html>
  ...
  <head>
    <title> Cart | FRIENDS CORNER CAFE</title>
  </head>
...
$gtotal = 0;
  ...
  foreach($_SESSION["cart"] as $keys => $values)
  {
	...
	$query4="INSERT INTO `order_details` (`order_id`, `f_id`, `quantity`, `amt`) VALUES ('".$order."', '".$p_id."', '".$quantity."', '".$total."');";	
...
<h2 class="text-center"> Thank you for Ordering at FRIENDS CORNER CAFE'! The ordering process is now complete. Here is Your Receipt </h2>
...
if(mysqli_num_rows($result) > 0)
{
  ...
  while($row = mysqli_fetch_array($result))
	{?>
		...
		$sql1="SELECT order_id,f_name,quantity,amt FROM order_details,food WHERE food.f_id=order_details.f_id AND order_id='".$order."';";
...
<?php
	while($row2=mysqli_fetch_array($result1))
	{
		...
		<?php
			if($row2["f_name"]=="COW MILK" ||$row2["f_name"]=="BUFFALO MILK")
			{
		?>
